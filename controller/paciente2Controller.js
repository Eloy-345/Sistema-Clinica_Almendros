...
const PacienteController = {
    obtenerTodos: async function (req, res) {
        await conexion.query('SELECT * FROM oftamologo', (error, tuplas) => {
            if (!error) {
                res.render('paciente/paciente2', { paciente: tuplas })
            } else {
    ...
    insertarAnalisis: async function (req, res) {
        ...
        await conexion.query('INSERT INTO oftamologo(hemoglobina,hematocrito,plaquetas,glucosa,urea,creatinina,rx,usg,medico,quirurgico,cirugiaP,cirugiaR,hallazgos, complicaciones,cirujano,ayudante,instrumentista,anestesiologo,circulante, esp1,esp2,diagnostico,elaboro) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [hemoglobina,hematocrito,plaquetas,glucosa,urea,creatinina,rx,usg,medico,quirurgico,cirugiaP,cirugiaR,hallazgos, complicaciones,cirujano,ayudante,instrumentista,anestesiologo,circulante, esp1,esp2,diagnostico,elaboro], (error, tuplas) => {
            if (error) {
                res.json({ status: 'error al insertar' })
            } else {
                res.redirect('/paciente2')
    ...
    actualizarPaciente: async function (req, res) {
        ...
        conexion.query('UPDATE oftamologo SET ide=?, nombre=?,email=?, telefono=?, edad=?,sexo=?,escolaridad=?,estado=?,dominancia=?,ocupacion=?,residencia=? WHERE id=?', [ide, nombre, email, telefono, edad, sexo, escolaridad, estado, dominancia, ocupacion, residencia, id], (error) => {
            if (error) {
