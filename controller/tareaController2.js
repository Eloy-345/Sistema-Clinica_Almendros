...
const AutosController = {
    obtenerTodos: async function (req, res) {
        await conexion.query('SELECT * FROM tarea2', (error, tuplas) => {
            if (!error) {
                res.render('tareas/tarea2', { alumnos: tuplas })
            } else {
    ...
    insertarTarea: async function (req, res) {
        ...
        await conexion.query('INSERT INTO tarea2(nombre,fecha,precio,hora,medico,color,paciente) values (?,?,?,?,?,?,?)', [nombre,fecha,precio,hora,medico,color,paciente], (error, tuplas) => {
            if (error) {
    ...
    actualizarTarea: async function (req, res) {
        ...
        conexion.query('UPDATE tarea2 SET nombre=?, fecha=?, precio=?, hora=?,medico=?, color=?, paciente=? WHERE id=?', [nombre,fecha,precio,hora,medico,color,paciente, id], (error)=>{
        if (error) {
