...
const AutosController = {
    obtenerTodos: async function (req, res) {
        await conexion.query('SELECT * FROM reporte3', (error, tuplas) => {
            if (!error) {
                res.render('agenda/agendaO', { alumnos: tuplas })
            } else {
    ...
    obtenerById: async function (req, res) {
        ...
        await conexion.query('SELECT *FROM reporte3 WHERE id= ?', [id], (error, tuplas) => {
            if (!error) {
                res.json(tuplas[0])
            } else {
    ...
    insertarTarea: async function (req, res) {
        ...
        await conexion.query('INSERT INTO reporte3(nombre,fecha,precio,hora,medico) values (?,?,?,?,?)', [nombre,fecha,precio,hora,medico], (error, tuplas) => {
            if (error) {
    ...
    actualizarTarea: async function (req, res) {
        ...
        conexion.query('UPDATE reporte3 SET nombre=?, fecha=?, precio=?, hora=?,medico=? WHERE id=?', [nombre,fecha,precio,hora,medico, id], (error)=>{
        if (error) {
    ...
    eliminarTarea: async function (req, res){
        const {id} = req.body
        await conexion.query('DELETE FROM reporte3 WHERE id = ?', [id], (error)=>{
        if (error) {
