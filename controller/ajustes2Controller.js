...
const AjustesController = {
    obtenerTodos: async function (req, res) {
        await conexion.query('SELECT * FROM user2', (error, tuplas) => {
            if (!error) {
                res.render('ajustes/ajustes2', { ajustes: tuplas })
            } else {
    ...
    obtenerById: async function (req, res) {
        ...
        await conexion.query('SELECT *FROM user2 WHERE id= ?', [id], (error, tuplas) => {
            if (!error) {
                res.json(tuplas[0])
            } else {
    ...
    actualizarAgenda: async function (req, res) {
        ...
        conexion.query('UPDATE user2 SET username=?, password=? WHERE id=?', [username,password,id], (error)=>{
        if (error) {
