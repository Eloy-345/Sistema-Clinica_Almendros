...
const NotaController = {
    obtenerTodos: async function (req, res) {
        await conexion.query('SELECT * FROM carta', (error, tuplas) => {
            if (!error) {
                res.render('nota_medica/carta', { alumnos: tuplas })
            } else {
    ...
    obtenerById: async function (req, res) {
        ...
        await conexion.query('SELECT *FROM carta WHERE id= ?', [id], (error, tuplas) => {
            if (!error) {
                res.json(tuplas[0])
            } else {
    ...
    eliminarNota: async function (req, res){
        const {id} = req.body
        await conexion.query('DELETE FROM carta WHERE id = ?', [id], (error)=>{
        if (error) {
