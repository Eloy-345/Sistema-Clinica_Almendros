...
const RecetaController = {
    obtenerTodos: async function (req, res) {
        await conexion.query('SELECT * FROM recetaO', (error, tuplas) => {
            if (!error) {
                res.render('receta/recetaO', { alumnos: tuplas })
            } else {
    ...
    obtenerById: async function (req, res) {
        ...
        await conexion.query('SELECT *FROM recetaO WHERE id= ?', [id], (error, tuplas) => {
            if (!error) {
                res.json(tuplas[0])
            } else {
    ...
    insertarReceta: async function (req, res) {
        const {nota} = req.body
        await conexion.query('INSERT INTO recetaO(nota) values (?)', [nota], (error, tuplas) => {
            if (error) {
    ...
    actualizarReceta: async function (req, res) {
        ...
        conexion.query('UPDATE recetaO SET nota=? WHERE id=?', [nota, id], (error)=>{
        if (error) {
    ...
    eliminarReceta: async function (req, res){
        const {id} = req.body
        await conexion.query('DELETE FROM recetaO WHERE id = ?', [id], (error)=>{
        if (error) {
